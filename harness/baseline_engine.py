...
class BaselineEngine:
    ...
    def evaluate_telemetry(self, telemetry: MultimodalTelemetrySnapshot) -> List[EvidenceItem]:
        ...
        # 5. Tire Fitment Quality Metrics
    ...
    def match_sops(self, evidence_list: List[EvidenceItem]) -> List[Dict[str, Any]]:
        ...
        """
        matched_sops = []
        evidence_text = " ".join([e.observation.lower() for e in evidence_list])

        for sop in self.sops:
            relevance_score = 0
            for symptom in sop.get("symptoms", []):
                keywords = [w for w in symptom.lower().split() if len(w) > 4]
        ...
        # Sort by relevance
        matched_sops.sort(key=lambda x: x["relevance_score"], reverse=True)
        return matched_sops
